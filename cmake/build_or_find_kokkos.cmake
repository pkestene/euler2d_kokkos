...
#    using FetchContent (which actually uses add_subdirectory)
# 2. If EULER2D_KOKKOS_BUILD is OFF (default), we don't build kokkos, but use
#    find_package for setup (you must have kokkos already installed)

# NOTE about required C++ standard we better chose to set the minimum C++
# standard level if not already done: - when building kokkos <  4.0.00, it
# defaults to c++-14 - when building kokkos >= 4.0.00, it defaults to c++-17 -
...
#
option(EULER2D_KOKKOS_BUILD
       "Turn ON if you want to build kokkos (default: OFF)" OFF)

#
# Option to use git (instead of tarball release) for downloading kokkos
#
option(
  EULER2D_KOKKOS_USE_GIT
  "Turn ON if you want to use git to download Kokkos sources (default: OFF)"
  OFF)
...
set_property(CACHE EULER2D_KOKKOS_BACKEND PROPERTY STRINGS "OpenMP" "Cuda"
                                                   "HIP" "Undefined")
...
if(EULER2D_KOKKOS_BUILD)
  ...
  if(EULER2D_KOKKOS_BACKEND MATCHES "Cuda")
    ...
    if((NOT DEFINED Kokkos_ENABLE_CUDA_CONSTEXPR)
       OR (NOT Kokkos_ENABLE_CUDA_CONSTEXPR))
  ...
  elseif(EULER2D_KOKKOS_BACKEND MATCHES "Undefined")
    ...
    message(
      FATAL_ERROR
        "[euler2d / kokkos] You must chose a valid EULER2D_KOKKOS_BACKEND !")
  ...
  if(EULER2D_KOKKOS_USE_GIT)
    ...
    message(
      "[euler2d / kokkos] Building kokkos from source using git submodule")
    FetchContent_Declare(
      kokkos_external
  ...
  if(TARGET Kokkos::kokkos)
    ...
    # set default c++ standard according to Kokkos version Kokkos >= 4.0.00
    # requires c++-17
    if(NOT "${CMAKE_CXX_STANDARD}")
    ...
    if(KOKKOS_DEVICE_ENABLE_CUDA)
      ...
      kokkos_check(OPTIONS CUDA_CONSTEXPR RETURN_VALUE
                   Kokkos_CUDA_CONSTEXPR_ENABLED)
